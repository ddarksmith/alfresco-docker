...
do
  ...
  if test -z ${lastId}; then
    ...
    curl -u admin:admin "http://alfresco:8080/alfresco/service/api/audit/query/alfresco-access?verbose=true&limit=5000&fromTime=${fromTime}" >> ${auditLog}
    lastId=`grep "\"id\":" ${auditLog} | tail -1 | sed s/,// | tr -d "\n" | tr -d "\r" | awk -F: '{print $2}'`
    echo "To Id = ${lastId}"
    lastId=$((lastId + 1))
  else
    echo "From Id: ${lastId}"
    curl -u admin:admin "http://alfresco:8080/alfresco/service/api/audit/query/alfresco-access?verbose=true&limit=5000&fromId=${lastId}" >> ${auditLog}
    lastId=`grep "\"id\":" ${auditLog} | tail -1 | sed s/,// | tr -d "\n" | tr -d "\r" | awk -F: '{print $2}'`
    echo "To Id = ${lastId}"
    lastId=$((lastId + 1))
  fi
